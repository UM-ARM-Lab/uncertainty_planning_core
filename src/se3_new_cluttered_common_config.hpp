...
#ifndef SE3_CLUTTERED_COMMON_CONFIG_HPP
#define SE3_CLUTTERED_COMMON_CONFIG_HPP
...
namespace se3_common_config
{
    inline common_config::OPTIONS GetDefaultOptions()
    {
        ...
        options.distance_clustering_threshold = 15.0 * options.environment_resolution; // 15.0 * options.environment_resolution;
        options.feasibility_alpha = 0.75;
        options.variance_alpha = 0.75;
        options.actuator_error = 0.125;
    ...
    inline std::pair<Eigen::Affine3d, Eigen::Affine3d> GetStartAndGoal()
    {
        ...
        const Eigen::Affine3d start = Eigen::Translation3d(5.0, 0.625, 2.5) * Eigen::Quaterniond::Identity();
        const Eigen::Affine3d goal = Eigen::Translation3d(5.0, 9.375, 2.5) * Eigen::Quaterniond::Identity();
...
#endif // SE3_CLUTTERED_COMMON_CONFIG_HPP
