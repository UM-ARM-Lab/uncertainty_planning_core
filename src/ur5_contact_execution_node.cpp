...
void peg_in_hole_env_linked(ros::Publisher& display_debug_publisher, ros::ServiceClient& robot_control_service, ros::ServiceClient& set_uncertainty_service)
{
    ...
    try
    {
        auto policy = uncertainty_planning_core::LoadUR5Policy(options.planned_policy_file);
        ...
        std::cin.get();
        const auto policy_simulation_results = uncertainty_planning_core::SimulateUR5UncertaintyPolicy(options, robot, sampler, policy, start_and_goal.first, start_and_goal.second, display_debug_publisher);
        ...
        std::function<std::vector<ur5_linked_common_config::SLC, std::allocator<ur5_linked_common_config::SLC>>(const ur5_linked_common_config::SLC&, const ur5_linked_common_config::SLC&, const double, const bool)> robot_execution_fn = [&] (const ur5_linked_common_config::SLC& target_configuration, const ur5_linked_common_config::SLC& expected_result_configuration, const double duration, const bool reset) { return move_robot(joint_names, target_configuration, expected_result_configuration, duration, reset, robot_control_service); };
        const auto policy_execution_results = uncertainty_planning_core::ExecuteUR5UncertaintyPolicy(options, robot, sampler, policy, start_and_goal.first, start_and_goal.second, robot_execution_fn, display_debug_publisher);
        ...
        uncertainty_planning_core::SaveUR5Policy(policy_execution_results.first, options.executed_policy_file);
